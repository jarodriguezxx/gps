...
import { Activity, Users, ClipboardList, FileBarChart, Apple, Search, FileText, ShoppingCart } from 'lucide-react';
...
const ExpedientesNutricion = () => {
  const navigate = useNavigate();
  const [activeNav, setActiveNav] = useState('expedientes');
  const [pacientesEvaluados, setPacientesEvaluados] = useState([]);
      ...
      try {
        ...
        if (resPacientes.ok) {
          ...
          for (let p of todos) {
            ...
            if (dataNutri && dataNutri.id) {
              conEvaluacion.push({ ...p, ultimaEvaluacion: dataNutri.fechaEvaluacion });
            }
          }
          setPacientesEvaluados(conEvaluacion);
        }
      } catch (error) {
  ...
  const filtrados = pacientesEvaluados.filter(p => p.nombreCompleto?.toLowerCase().includes(busqueda.toLowerCase()));
              ...
              <section className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
                <div className="flex flex-col md:flex-row md:items-center justify-between p-5 border-b border-slate-200 gap-4 bg-slate-50/50">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h2 className="text-base font-black uppercase tracking-[0.2em] text-slate-700">Archivo de Expedientes</h2>
                      <p className="text-xs text-slate-500 mt-0.5">Consulte el historial nutricional de los pacientes</p>
                    </div>
                  </div>
                  <div className="relative flex-1 md:max-w-md">
                    <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
                    <input type="text" placeholder="Buscar expediente..." value={busqueda} onChange={(e) => setBusqueda(e.target.value)} className="w-full pl-9 py-2 rounded-xl border border-slate-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-[#7E1D3B]/30" />
                  </div>
                </div>
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-slate-50 border-b border-slate-200 text-[11px] font-black uppercase tracking-[0.15em] text-slate-500">
                        ...
                        <th className="px-6 py-4">Última Evaluación</th>
                        <th className="px-6 py-4 text-right">Acción</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-200">
                      {loading ? (
                        <tr><td colSpan="4" className="py-12 text-center text-slate-500 text-sm">Cargando archivo...</td></tr>
                      ) : filtrados.length === 0 ? (
                        <tr><td colSpan="4" className="py-12 text-center text-slate-500 text-sm">No hay expedientes nutricionales guardados.</td></tr>
                      ) : (
                        filtrados.map(p => (
                          <tr key={p.id} className="hover:bg-slate-50 transition-colors">
                            ...
                            <td className="px-6 py-4 text-sm text-slate-600 font-medium">{p.ultimaEvaluacion || 'Fecha no registrada'}</td>
                            <td className="px-6 py-4">
                              <div className="flex items-center justify-end gap-2">
                                <button onClick={() => navigate(`/nutriologo/vista-expediente/${p.id}`)} className="px-3 py-2 bg-slate-100 text-slate-600 rounded-lg hover:bg-slate-200 text-xs font-bold transition-colors flex items-center gap-1.5 border border-slate-200 shadow-sm">
                                  <FileText size={14} /> Abrir Expediente
                                </button>
                              </div>
                            </td>
                          </tr>
...
export default ExpedientesNutricion;