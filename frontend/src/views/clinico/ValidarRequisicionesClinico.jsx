...
import { AdminHeader, ClinicoSidebar } from '../../components/layout/AdminLayout';
...
export default function ValidarRequisicionesClinico() {
  ...
  const cargar = useCallback(async () => {
    ...
    try {
      ...
      const deClinico = Array.isArray(data)
        ? data.filter(req => req.area?.toLowerCase() === 'clínico' || req.area?.toLowerCase() === 'clinico')
        : [];
      setRequisiciones(deClinico);
    } catch {
      setRequisiciones([]);
    } finally {
      setCargando(false);
  ...
  const handleValidar = async (req) => {
    setProcesando(true);
    try {
      const res = await fetch(`${API_BASE}/requisiciones/${req.id}/validar-clinico`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', 'X-Rol': 'jefe-clinico' },
      });
      if (!res.ok) {
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-900">
      <div className="mx-auto w-full max-w-7xl px-4 py-4 md:px-6">
        <AdminHeader submodule="Clínico — Validar Requisiciones" />
            ...
            <div className="flex items-start justify-between mb-5">
              <div>
                ...
                <p className="text-sm text-slate-400 font-medium tracking-wide">
                  Aprueba o rechaza las solicitudes pendientes del área Clínica
                </p>
              </div>
              {pendientesCount > 0 && (
