...
import { AdminHeader, MedicoSidebar } from '../../components/layout/AdminLayout';
...
export default function ValidarRequisionesMedico() {
  ...
  const cargar = useCallback(async () => {
    ...
    try {
      ...
      const deMedico = Array.isArray(data)
        ? data.filter(req => req.area?.toLowerCase() === 'médico' || req.area?.toLowerCase() === 'medico')
        : [];
      setRequisiciones(deMedico);
    } catch {
      setRequisiciones([]);
    } finally {
      setCargando(false);
  ...
  const handleValidar = async (req) => {
    setProcesando(true);
    try {
      const res = await fetch(`${API_BASE}/requisiciones/${req.id}/validar-medico`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', 'X-Rol': 'jefe-medico' },
      });
      if (!res.ok) {
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-900">
      <div className="mx-auto w-full max-w-7xl px-4 py-4 md:px-6">
        <AdminHeader submodule="Médico — Validar Requisiciones" />
            ...
            <div className="flex items-start justify-between mb-5">
              <div>
                ...
                <p className="text-sm text-slate-400 font-medium tracking-wide">
                  Aprueba o rechaza las solicitudes pendientes del área Médica
                </p>
              </div>
              {pendientesCount > 0 && (
                ...
                <div className="px-6 py-4 border-t border-slate-100 flex gap-3 justify-end shrink-0">
                  {!modoRechazo ? (
                    <>
                      ...
                      <button onClick={() => handleValidar(seleccionada)} disabled={procesando}
