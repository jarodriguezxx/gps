...
export default function ValidarRequisicionesArea({
  areaNombre,
  areaFiltro,
  rolHeader,
  validatePath,
  ...
  const handleValidar = async (req) => {
    setProcesando(true);
    try {
      const res = await fetch(`${API_BASE}${validatePath.replace(':id', req.id)}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', 'X-Rol': rolHeader },
      });
      if (!res.ok) {
        mostrarMsg('error', (await res.text()) || 'No se pudo validar la requisición.');
      } else {
        mostrarMsg('ok', 'Requisición validada y enviada a la siguiente etapa.');
        cerrarModal();
        await cargar();
      }
    } catch {
  ...
  const filtradas = requisiciones.filter(r => filtro === 'TODAS' ? true : r.estado === filtro);
  const pendientesCount = requisiciones.filter(r => r.estado === 'PENDIENTE').length;

  return (
    <div className="min-h-screen bg-slate-100 text-slate-900">
      <div className="mx-auto w-full max-w-7xl px-4 py-4 md:px-6">
        <AdminHeader submodule={`${areaNombre} — Validar Requisiciones`} />
        ...
        <main className="space-y-5">
          <div className="grid gap-4 md:grid-cols-[220px_1fr]">
            ...
            <div className="space-y-5 min-w-0">
              <div className="flex items-start justify-between mb-5">
                <div>
                  ...
                  <p className="text-sm text-slate-400 font-medium tracking-wide">
                    Aprueba o rechaza las solicitudes pendientes del área de {areaNombre}
                  </p>
                </div>
                {pendientesCount > 0 && (
              ...
              {msg && (
                <div className={`mb-4 rounded-xl px-4 py-3 text-sm font-semibold border ${msg.tipo === 'ok' ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-rose-50 border-rose-200 text-rose-700'}`}>
                  {msg.texto}
                </div>
              )}
              ...
              <div className="flex gap-2 mb-5 flex-wrap">
                {[
                  ...
                  <button
                    ...
                    className={`px-4 py-1.5 rounded-xl text-xs font-bold border transition ${filtro === f.key ? 'bg-[#7E1D3B] text-white border-[#7E1D3B] shadow-sm' : 'bg-white border-slate-200 text-slate-500 hover:border-[#7E1D3B]/40 hover:text-[#7E1D3B]'}`}
                  >
                    {f.label}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 shrink-0">
              ...
              <button onClick={cerrarModal} className="p-2 rounded-xl hover:bg-slate-100 text-slate-400 hover:text-slate-600 transition">
                <X size={18} />
              </button>
            </div>
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {seleccionada.articulos.map((art, i) => (
                          <tr key={art.id || i} className="bg-white">
                            <td className="px-3 py-2.5 text-slate-700 font-medium">{art.articuloRequisitado || art.producto || art.descripcion || '—'}</td>
                            <td className="px-3 py-2.5 text-center text-slate-500">{art.unidad}</td>
                            <td className="px-3 py-2.5 text-center text-slate-700 font-bold">{art.articulosSolicitados ?? art.cantidad ?? '—'}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
